...
export function fallo(error: unknown, contexto: string): NextResponse {
  ...
  const esDominio = error instanceof Error && ['OrderError', 'ShippingNotConfiguredError', 'PaymentNotConfiguredError', 'PaymentMethodNotSupportedError', 'CodNotAvailableError'].includes(error.name);
